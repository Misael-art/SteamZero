...
Item {
    ...
    readonly property var selectedGame: scene && scene.items
        ? scene.items[Number(scene.selected || 0)] || ({}) : ({})
    property var accessibility: ({})
    readonly property var entries: scene && scene.layouts && scene.layouts.covers
        ? scene.layouts.covers.entries : []
    ...
    Text {
        ...
        text: cinema.selectionReady ? String(cinema.selectedGame.title || "")
                                   : qsTr("Atualizando seleção…")
        color: "#ffffff"
