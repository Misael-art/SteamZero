...
//
// VS-03 — cenário de captura de CENA pertencente ao projeto.
//
// Contraparte de `CaptureHarness.qml` para uma cena composta: cada nó em
// `config.nodes` é um modelo renderizado por SceneText.qml ou SceneImage.qml
// conforme `kind`. O relatório geométrico é uma LISTA, um item por nó.
//
// `mediaFiles` faz o mesmo papel de `CaptureImageHarness.qml`: o test-double
// do mapeamento de assets do shell, chave `assets/...` -> arquivo real.
import QtQuick
        ...
        delegate: Loader {
            ...
            function loadNode() {
                if (nodeData === undefined)
                    return
                var url = nodeData.kind === "image" ? stage.imageSource : stage.textSource
