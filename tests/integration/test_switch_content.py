...
def test_remove_keeps_shared_blob_until_last_catalog_record(
    ...
    first = manager.plan_import(source, kind="update", title_id="0100000000010000", version="1.0.0")
    assert first.plan is not None
    manager.apply_import(first.plan.plan_id, first.plan.confirm_token)
    second = manager.plan_import(source, kind="dlc", title_id="0100000000010000", version="pack-a")
    ...
    assert [record.record_key for record in manager.list_records()] == [second.record.record_key]
