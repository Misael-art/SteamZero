...
| 1 | Isolamento de testes (GAP-G26) | **fechado no software** — suíte integral isolada e state real preservado | diagnóstico a41 | suíte integral com cinco homes XDG temporários e zero mudança no state real | P0 | preservar o runner e não limpar state histórico sem autorização |
| 2 | Recovery, doctor e state audit (GAP-G25) | **parcial** — software integrado; host ainda tem 1 job stale pós-boot | GAP-G26 para prova confiável | restart recupera uma vez; doctor/audit detectam incoerência; cleanup só por plano/quarentena | P0 | ciclo governado autorizado e confirmação de `staleJobs=0` |
| 3 | Verdade de componentes (GAP-G27) | **parcial verificado** — matriz por origem concorda; xenia-canary permanece degradado | gates isolados | AppImage/Flatpak roteados por origem; CLI/workspace/QML concordam; sem fallback silencioso | P1 | prova física autorizada de lançamento, sessão e retorno ao Launcher |
| 4 | Verdade de mídia (GAP-G28) | **parcial verificado** — health e quota chegam à UI; host aguarda credencial e recovery do job stale | recovery de jobs | saúde persistida; quota visível; uma tentativa por job após esgotamento | P1 | configurar credencial pessoal e repetir busca autorizada após recovery |
| 5 | Prontidão GameMode (GAP-G29) | **parcial verificado** — binário presente, daemon/autorização/efeitos não observáveis no host | projeção honesta | binário, daemon, autorização e efeitos separados; nenhuma mutação administrativa automática | P1 | validação autorizada em sessão real, sem aplicar plano automaticamente |
| 6 | Recursos e probe QML (GAP-G30/G31) | **parcial verificado** — atribuição e guardas Qt integradas; host sem procfs completo | runner/gates isolados | PSS atribuído; retorno Qt validado; zero novo aborto do probe | P1/P2 | repetir medição em runner/sessão com permissão suficiente |
...
| 8 | Fechar P0-03 | **parcial** — a fatia traduz 13 atributos (12 de texto e `image.src`, incluindo `layer`→`zIndex`); corpus tem 388 propriedades | a42 estabilizada | 388 migradas, cobertura 100%, relatório | P1 | migrar corpus por família |
| 9 | Árvore de cena e texto avançado | **parcial** — `children` e round-trip v2 já existem; wrapping, elide, limite de linhas, auto-fit e `StyledText` allowlisted integrados à fatia; migração do corpus permanece aberta | P0-03 | children, wrapping, elide, rich text, auto-fit com gates | P1 | declarar formato nos itens do corpus e ampliar a cobertura |
| 10 | Acessibilidade real | **parcial** — host `visualScale`, `reducedMotion` e `highContrast` chegam a consumidores; Main, Emulation, SteamGameplay, SteamDesktop, EditorialLibrary/ScreenshotRail, LauncherGamePage, ThemeCatalogPanel/ThemeSceneFullscreen, ThemeEditorPanel/ThemeStudioCanvas/SceneSurfacePreview, ColorPickerDialog, SceneRepeater/SceneText/SceneBadge, SectionNavigator/SectionMenu/LoadingOverlay/EmptyState/FeedbackNotice e quatro cartões reutilizáveis escalam tamanhos fixos; a ponte do shell publica snapshot/generation, a cena default aplica `accessibility.visualScale` a todos os seus textos e o emissor RetroFE da fatia vertical também declara o binding; LauncherShell/LauncherMain/LauncherHome e SceneEsdeView permanecem sob ownership ativo com pixels fixos; a propagação para todos os contextos e componentes reutilizáveis permanece aberta (GAP-G12/G15) | tipografia/layout | textScale, reducedMotion, highContrast consumidos de verdade em todas as superfícies | P1 | propagar contexto/helper aos componentes restantes e medir a cena completa |
| 11 | Migração dos harnesses QML | **parcial no gate RHI** — handheld, jornadas/inventário UI e os quatro probes reais do Launcher agora falham explicitamente quando Qt/SVG/qmltestrunner falta; não restam `skipif` nos probes QML alvo | harness canônico | zero skip no gate visual | P2 | fechar a matriz RHI/P0-08 e confirmar o job visual canônico |
...
| GAP-G13 | probes QML legados usavam `skipif` | **P2** | falso verde possível | módulos alvo agora falham explicitamente no gate visual; skip RHI específico permanece em P0-08 | fechar a matriz RHI canônica | zero skip ambiental no gate visual |
| GAP-G14 | ~~baselines não versionadas~~ | — | **FECHADA** em `6fb2a75` | — | — | — |
| GAP-G15 | geração de acessibilidade sem consumidor | **P2** | ponte declarativa incompleta e recomputação da cena completa não provada | bindings `accessibility.*` + `fontScale` no Resolver | propagar snapshot e medir cena completa | recomputação seletiva medida |
...
| ~~GAP-G22~~ | ~~benchmark de 10 mil arquivos usava teto de tempo de parede em runner compartilhado~~ | **FECHADA em 2026-07-30** — o teto absoluto foi removido; `--durations=20` + JUnit XML publicam o tempo real sem bloquear o CI por carga do runner | asserções funcionais mantidas | medição publicada por versão Python; não estabelece baseline entre máquinas | repetição controlada exige runner dedicado |
