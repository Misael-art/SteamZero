...
| 10 | Acessibilidade real | **parcial** — host `visualScale`, `reducedMotion` e `highContrast` chegam a consumidores; Main, Emulation e SteamGameplay agora escalam tamanhos fixos, mas componentes reutilizáveis e a invalidação `a11y` do Resolver permanecem abertos (GAP-G12/G15) | tipografia/layout | textScale, reducedMotion, highContrast consumidos de verdade em todas as superfícies | P1 | propagar helper aos componentes restantes e ligar uma propriedade declarativa à geração `a11y` |
