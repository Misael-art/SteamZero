# Radiografia consultiva priorizada — reauditoria 2026-09-07

## 1. Veredito executivo

Esta reauditoria foi executada contra a release instalada
`2.0.0rc1-b09908a58261` (origem `b09908a5826174b8ff8d02fbf09f7dcef288edd9`),
com `steamzero-host converge` idempotente e sem reiniciar a sessão KDE. O
SteamZero tem um núcleo operacional honesto e um AURA Launcher já capaz de
...
| Theme Studio | 6 | Canvas, árvore, inspector e edição declarativa foram instalados; contraste do painel claro foi provado visualmente, mas input físico/persistência ainda não. |
...
| SZ-THEME-STUDIO | `C` canvas/árvore/inspector e edição declarativa de layout; `P` interação física e reabertura | `GAP-THEME-STUDIO-PHYSICAL-CANVAS` confirmado como lacuna de input/persistência; contraste visual entregue em `2026-09-07-theme-studio-contrast` |
...
| H4 | Parcial | O canvas instalado deixou de ser somente leitura: propriedades declarativas de layout têm allowlist, validação e persistência no save; a alteração por input físico continua não validada. |
...
| H7 | Parcial | A correção de contraste do inspector do Theme Studio está instalada e legível; permanecem os demais gaps da Central (catálogo de temas/bridge, órfãos e roteamento live-launcher). |
...
Depois, obter input Wayland para provar a mutação do Theme Studio e fazer a
entrada Big Picture e o retorno jogo→launcher terem prova física completa. A
edição declarativa já está implementada; os tipos ES-DE
carousel/textList/video/helpSystem ainda devem ser ampliados somente quando a
navegação integrada existir — um preview bonito sem uso real cria expectativa
maior que a capacidade entregue.
