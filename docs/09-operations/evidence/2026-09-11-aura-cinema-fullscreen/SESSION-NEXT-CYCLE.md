...
## Contrato de confirmação implementado (pré-instalação)

O plano do `/tmp` (reconstruído em desenho de trabalho fora do repositório) foi implementado em quatro passos com testes de subprocesso reais: exceção tipada `LaunchNotStartedError` envolvendo somente a fase de preparação; acknowledgment `notStarted` no envelope do CLI; adapter `launcher_receipt` capturando a resposta em worker próprio (estados `confirmed`/`notStarted`/`unconfirmed`, `delayed` transitório, sem matar o jogo e sem prender HTTP); `LaunchRouter` com requestId por pedido e rota Steam fora do contrato; bridge publicando requestId no `/launch` (agora 200), projeção allowlisted da tentativa em `/session` e liberação da tentativa em `notStarted` confirmado — `unconfirmed` segue travado por design. O shell QML só aceita falha sem sessionId com o requestId do POST atual e o estado `failed` mantém o botão "Voltar e tentar" operável por teclado.

Prova negativa registrada: com o wrap e o branch do CLI revertidos, exatamente os dois testes novos de acknowledgment reprovam. Gate integral em `14-launch-confirmation-gates.xml`: 5.953 passed, 47 skipped e 1 reprovação de consistência de visões (regenerada; `status-check` OK, 10 testes de status). Bateria focada: 31 testes de recibo/CLI/app, 64 na bateria do launcher, `check_launcher_shell.qml` com 7 verificações novas. Commits pendentes de aplicação pelo bloqueio do gate Mimosa (highs pré-existentes em `game_stream.py` e `reference/`, fora do diff); prova física na release instalada permanece o fechamento do ciclo.
