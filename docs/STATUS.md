...
| SZ-EMULATION-M10 | Ciclo transacional de emuladores Flatpak | verified-vm | partial | feature-branch | vm | degraded | installed | Abrir o RetroArch uma vez no host para que ele declare joypad_autoconfig_dir (sem isso nenhum perfil pode ser aplicado), decidir a fonte de indice para pads sem autoconfig empacotado (o pad interno deste Deck e um deles) e provar install/verify/rollback de um emulador no host. |
