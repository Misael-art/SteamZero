...
| SZ-EMULATION-M10 | Ciclo transacional de emuladores Flatpak | verified-vm | partial | feature-branch | vm | degraded | installed | Fechar a G45: provar install/verify/rollback de pelo menos um emulador NO HOST. A certificacao em VM cobriu o ciclo com commits validos a epoca; o pino do RetroArch ja nao resolve no Flathub. |
...
| SZ-MEDIA-SCRAPING | Metadados e midia por scraping controlado | implemented | complete | integrated | unit | degraded | installed | Fechar a G44: distinguir 'cofre ausente ou efemero' de 'credencial nao preenchida' e dizer qual provedor de segredo esta atendendo; hoje os dois casos aparecem iguais na tela. Depois, fechar a G40 (systemeid do ScreenScraper). |
