...
| SZ-CAST-LAN | Partilha de ecra em rede local | implemented | complete | integrated | unit | degraded | installed | Expor a configuracao do orquestrador na UI (G41): hoje discover responde vazio e status diz 'Orquestrador nao configurado', sem caminho para configurar. Depois, prova fim a fim em dois dispositivos LAN reais (operador). |
...
| SZ-FRONTEND-STEAM-SHORTCUTS | Atalhos do Steam (shortcuts.vdf) transacionais | implemented | complete | integrated | unit | unknown | not-packaged | Exercitar contra um shortcuts.vdf real do operador antes de promover a verificacao integrada. |
