...
| SZ-EMULATION-ENHANCEMENTS | Melhorias por jogo agnósticas de plataforma | implemented | complete | feature-branch | unit | unknown | not-packaged | Validacao fisica pendente: nenhum dos 5 emuladores alvo (DuckStation, PCSX2, Dolphin, RPCS3, Cemu) esta instalado no host, e a biblioteca real tem apenas jogos de Switch. Instalar um emulador alvo + ROM correspondente e lancar pela release instalada. Pendente tambem: nomes por identidade em dolphin-gameini e cemu-rules (hoje fixos), por isso esses dois manifestos nao declaram enhancements. |
