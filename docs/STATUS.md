...
| SZ-EMULATION-M10 | Ciclo transacional de emuladores Flatpak | verified-vm | partial | feature-branch | vm | degraded | installed | Definir e provar a integracao Flatpak: o RetroArch declara joypad_autoconfig_dir=/app/share/libretro/autoconfig (interno ao sandbox, inexistente no host) e le perfis em <dir>/<input_joypad_driver>/. Abrir o RetroArch NAO destrava. --appendconfig funciona mas config_save_on_exit=true persiste o valor no arquivo usado. |
